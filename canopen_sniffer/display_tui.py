...
"""!
@file display_tui.py
@brief Textual-based TUI frontend for live CANopen monitoring.
@details
Implements a rich, interactive terminal UI using Textual that renders:
 - Protocol Data tables (decoded PDO/SDO content)
 - Bus Stats tables and rolling graphs
 - Command input for limited interactive commands (single-line, 50 char width)

The TUI run loop is blocking; integration into larger applications should
run the TUI in a separate thread or process. This module documents the
expected frame format and the public entrypoints used by the main sniffer
application (run_textual, stop).
"""
    ...
    def run_textual(cls, stats_instance, processed_frame_queue, fixed=False):
        ...
        class _TUI(App):
            ...
            def __init__(self, *a, **kw):
                ...
                self.proto_display = [copy.deepcopy(blank_proto) for _ in range(sniffer_defs.PROTOCOL_TABLE_HEIGHT)]

                ## PDO data display buffer
                self.pdo_display = [copy.deepcopy(blank_pdo) for _ in range(sniffer_defs.DATA_TABLE_HEIGHT)]

                ## SDO data display buffer
                self.sdo_display = [copy.deepcopy(blank_sdo) for _ in range(sniffer_defs.DATA_TABLE_HEIGHT)]
            ...
            def _copy_to_clipboard_or_file(self, text: str, filename: str = f"/tmp/{sniffer_defs.APP_NAME}.log"):
                """! Try to copy to clipboard using pyperclip; if unavailable, write to filename."""
                try:
                    pyperclip.copy(text)
                    return True, "Copied to clipboard"
                except Exception:
                    self.log.exception(f"Failed in copying table to clipboard, using temp file <{filename}> for fallback")
                    ...
                    try:
                        ...
                        self.log.exception(f"Failed to copy/write: {e}")
            ...
            async def on_mount(self) -> None:
                ...
                self.log.info("display_tui mounted")
                ## Title to display on TUI console.
                self.title = sniffer_defs.APP_ORG

                ## Sub title to display on TUI console.
                self.sub_title = sniffer_defs.APP_NAME
                ...
                self.bus_stats_table.add_column("Graph", width=sniffer_defs.STATS_GRAPH_WIDTH)

                # enforce fixed visual heights so DataTable doesn't expand indefinitely
                try:
                    # add header row height cushion (~3) and a small margin
                    self.proto_table.height = max(3, sniffer_defs.PROTOCOL_TABLE_HEIGHT)
                    self.pdo_table.height = max(3, sniffer_defs.DATA_TABLE_HEIGHT)
                    self.sdo_table.height = max(3, sniffer_defs.DATA_TABLE_HEIGHT)
            ...
            def sparkline_text(self, history, width=None):
                ...
                if not history:
                    return ""
                try:
                    seq = list(history)[- (width or sniffer_defs.STATS_GRAPH_WIDTH):]
                    if not seq:
                        return ""
                    blocks = "▁▂▃▄▅▆▇█"
            ...
            def _update_from_queue(self) -> None:
                ...
                while True:
                    try:
                        ...
                        break
                    got_any = True
                    t = pframe.get("time", sniffer_defs.now_str())
                    ...
                    type_name = ftype.name if isinstance(ftype, sniffer_defs.frame_type) else str(ftype)

                    # Consistently use cls.fixed (set by run_textual) to decide behavior.
                    if cls.fixed:
                        if ftype == sniffer_defs.frame_type.PDO:
                            key = (cob, idx, sub)
                        ...
                        elif ftype in (sniffer_defs.frame_type.SDO_REQ, sniffer_defs.frame_type.SDO_RES):
                            key = (cob, idx, sub)
                            row = {"time": t, "cob": cob_s, "name": name, "index": idx_s, "sub": sub_s, "raw": raw, "decoded": str(decoded), "count": 1}
            ...
            def _refresh_tables(self):
                ...
                if cls.fixed:
                    ...
                    protos = real_protos_sorted[:sniffer_defs.PROTOCOL_TABLE_HEIGHT]
                    remaining = sniffer_defs.PROTOCOL_TABLE_HEIGHT - len(protos)
                    if remaining > 0:
                        protos.extend(blank_protos[:remaining])
                        while len(protos) < sniffer_defs.PROTOCOL_TABLE_HEIGHT:
                ...
                if cls.fixed:
                    ...
                    pdos = real_pdos_sorted[:sniffer_defs.DATA_TABLE_HEIGHT]
                    remaining = sniffer_defs.DATA_TABLE_HEIGHT - len(pdos)
                    if remaining > 0:
                        pdos.extend(blank_pdos[:remaining])
                        while len(pdos) < sniffer_defs.DATA_TABLE_HEIGHT:
                ...
                if cls.fixed:
                    ...
                    sdos = real_sdos_sorted[:sniffer_defs.DATA_TABLE_HEIGHT]
                    remaining = sniffer_defs.DATA_TABLE_HEIGHT - len(sdos)
                    if remaining > 0:
                        sdos.extend(blank_sdos[:remaining])
                        while len(sdos) < sniffer_defs.DATA_TABLE_HEIGHT:
            ...
            def _refresh_bus_stats(self):
                ...
                # Top talkers
                try:
                    top = snapshot.top_talkers.most_common(sniffer_defs.MAX_STATS_SHOW)
                ...
                try:
                    ...
                    shown = items[:sniffer_defs.MAX_STATS_SHOW]
