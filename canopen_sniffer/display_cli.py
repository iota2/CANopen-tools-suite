...
"""!
@file display_cli.py
@brief Simple CLI (Rich) display backend for the CANopen sniffer.
@details
This module contains a non-interactive command-line display backend used
for quick inspection and automated piping to files. It consumes the
`processed_frame` queue and expects each frame to be a dict with keys such
as: timestamp, can_id, dlc, data, canopen_type, parsed (optional).

Public API:
 - run(): blocking method that renders periodic summaries to stdout.
 - stop(): triggers graceful shutdown.

This backend is intentionally minimal and suited for headless operation or
CI-style captures where a TUI/GUI is not required.
"""
...
class display_cli(threading.Thread):
    ...
    def __init__(self, stats: bus_stats, processed_frame: queue.Queue, fixed: bool = False):
        ...
        self.proto_frames = deque(maxlen=sniffer_defs.MAX_FRAMES)

        ## PDO data buffer used only for rendering rows (not for rate calc).
        self.pdo_frames = deque(maxlen=sniffer_defs.MAX_FRAMES)

        ## SDO data buffer used only for rendering rows (not for rate calc).
        self.sdo_frames = deque(maxlen=sniffer_defs.MAX_FRAMES)
    ...
    def sparkline(self, history, style="white"):
        ...
        try:
            seq = list(history)[-sniffer_defs.STATS_GRAPH_WIDTH:]
            if not seq:
                return ""
            blocks = "▁▂▃▄▅▆▇█"
    ...
    def build_bus_stats_table(self):
        ...
        graph_col_width = sniffer_defs.STATS_GRAPH_WIDTH
    ...
    def render_tables(self):
        ...
        protos = list(self.fixed_proto.values())[-sniffer_defs.PROTOCOL_TABLE_HEIGHT:] if self.fixed else list(self.proto_frames)[-sniffer_defs.PROTOCOL_TABLE_HEIGHT:]
        ...
        frames = list(self.fixed_pdo.values())[-sniffer_defs.DATA_TABLE_HEIGHT:] if self.fixed else list(self.pdo_frames)[-sniffer_defs.DATA_TABLE_HEIGHT:]
        while len(frames) < sniffer_defs.DATA_TABLE_HEIGHT:
        ...
        sdos = list(self.fixed_sdo.values())[-sniffer_defs.DATA_TABLE_HEIGHT:] if self.fixed else list(self.sdo_frames)[-sniffer_defs.DATA_TABLE_HEIGHT:]
