...
"""!
@file bus_stats.py
@brief High-resolution bus statistics for CANopen sniffer.
@details
This module implements a thread-safe bus statistics helper used by the
CANopen sniffer application to collect, aggregate and expose bus-level
metrics for GUI/TUI display and CSV export.

Primary responsibilities:
 - Counting frames by CANopen message type
 - Tracking cumulative payload bytes for throughput estimation
 - Recording SDO request/response timings and response histograms
 - Computing frames-per-second rates and bus utilization estimates
 - Offering a copy-on-read snapshot API suitable for UI rendering

Thread-safety: all public mutating methods acquire an internal lock so this
object is safe for concurrent use by sniffer and processing threads.
"""
...
class bus_stats:
    ...
    class frame_count:
        ...
        counts : dict = field(default_factory=lambda: dict.fromkeys(sniffer_defs.frame_type, 0))
    ...
    class payload_size:
        ...
        sizes: dict = field(default_factory=lambda: {sniffer_defs.frame_type.PDO: 0, sniffer_defs.frame_type.SDO_RES: 0})
    ...
    class sdo_stats:
        ...
        response_time: deque = field(default_factory=lambda: deque(maxlen=sniffer_defs.STATS_GRAPH_WIDTH * 5))
    ...
    def __init__(self, bitrate: int = sniffer_defs.DEFAULT_CAN_BIT_RATE):
        ...
        self._stats.rates.history = {k: deque(maxlen=sniffer_defs.STATS_GRAPH_WIDTH) for k in keys}
    ...
    # --------- Update helpers ---------
    def increment_frame(self, ftype: sniffer_defs.frame_type):
    ...
    def increment_payload(self, ftype: sniffer_defs.frame_type, size: int):
    ...
    def update_sdo_request_time(self, index: int, sub: int):
        ...
        sniffer_defs.log.debug(f"SDO request idx=0x{index:04X} sub={sub} recorded for latency measurement")
    ...
    # --------- Getters ---------
    def get_frame_count(self, ftype: sniffer_defs.frame_type) -> int:
        """! Get counted frames.
    ...
    def update_rates(self, now: float = None, interval: float = 1.0):
        ...
        with self._lock:
            ...
            counts['hb'] = self._stats.frame_count.counts.get(sniffer_defs.frame_type.HB, 0)
            counts['emcy'] = self._stats.frame_count.counts.get(sniffer_defs.frame_type.EMCY, 0)
            counts['pdo'] = self._stats.frame_count.counts.get(sniffer_defs.frame_type.PDO, 0)
            counts['sdo_res'] = self._stats.frame_count.counts.get(sniffer_defs.frame_type.SDO_RES, 0)
            counts['sdo_req'] = self._stats.frame_count.counts.get(sniffer_defs.frame_type.SDO_REQ, 0)

            # compute deltas and rates in a loop
            keys = self._stats.rates.keys
            # ensure history dict exists
            if not getattr(self._stats.rates, "history", None):
                width = getattr(self, "_rate_history_width", sniffer_defs.STATS_GRAPH_WIDTH)
            ...
            for k in keys:
                ...
                if k not in rh:
                    rh[k] = deque(maxlen=sniffer_defs.STATS_GRAPH_WIDTH)
                rh[k].append(rate)
            ...
            try:
                ...
                pdo_payload = self._stats.payload_size.sizes.get(sniffer_defs.frame_type.PDO, 0)
                sdo_payload = self._stats.payload_size.sizes.get(sniffer_defs.frame_type.SDO_RES, 0) + self._stats.payload_size.sizes.get(sniffer_defs.frame_type.SDO_REQ, 0)
