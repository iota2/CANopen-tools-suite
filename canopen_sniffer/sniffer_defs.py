...
"""
@file sniffer_defs.py
@brief Shared constants and small utility helpers for the CANopen sniffer.
@details
Defines CLI defaults, format helpers and small utility functions used across
the codebase. Functions such as `now_str`, `bytes_to_hex` and
`clean_int_with_comment` provide normalized, tested outputs for display and
CSV export.

Constants are intentionally grouped to be imported with `from ... import *`
for convenience in the CLI modules.
"""
